...
Астрологическое мобильное приложение для построения натальных карт и гороскопов. Проводит пользователя через 4 экрана (дата рождения → имя → город → карта предсказаний) и показывает ежедневный прогноз на основе транзитных аспектов.
...
│   ├── screens/              # Экраны (BirthDate, Name, City, Prediction)
...
Приложение проходит пользователя через 4 экрана (`SCREEN_ORDER` в `App.js`):
