...
Процесс бэкенда, который запускается в ../nm-astrology командой air не должен быть занят. Если ты его запускаешь для проверки, то нужно остановить, чтобы я мог запустить его сам.
